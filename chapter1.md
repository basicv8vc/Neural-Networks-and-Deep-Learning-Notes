...
感知机不仅可以用于做出决策，还可以用来表示逻辑电路中基本的门电路，例如AND, OR, NAND。

举个例子，

![](https://ooo.0o0.ooo/2015/11/09/56415c11139a1.png)

上图中的感知机有两个输入变量，权重均为-2。如果令$$x_{1},x_{2}$$取值为0，则$$(-2)*0+(-2)*0+3=3$$,因此感知机输出1；如果令$$x_{1},x_{2}$$取值为1，则$$(-2)*1+(-2)*1+3=-1$$,因此感知机输出0。

看到没，输入00，输出1；输入11，输出0，这不就是

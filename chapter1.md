...
作者给出了一种启发式想法：对于当前的分类器，她的任务是识别0-9共10个数字，最直观的做法就是输出层含有10个神经元，每个神经元分别识别相对应的数字就可以了，比如，第一个神经元$$o_{1}$$的作用是专门识别图像数字是否是0，假设隐含层含有4个神经元，$$o_{1}$$要根据隐含层中的四部分输出决定图像是否为0，更进一步，我们想要知道隐含层这4个神经元是干嘛的，是否可以这样假设：每一个数字都可以拆成四部分，每个神经元分别判断自己负责的那部分图像，比如0可以拆成

![](https://ooo.0o0.ooo/2015/11/10/5641ded88def8.png)
![](https://ooo.0o0.ooo/2015/11/10/5641deef07626.png)

隐含层每一个神经元分别给出输入图像中自己负责的部分是否符合数字的特征，然后输出层的神经元根据隐含层提供的信息给出最终的决策。如果这样想的话，输出层使用4个神经元就不合适了，因为此时每个神经元需要根据隐含层的输入判断输入图像是哪两个数字，这显然很困难。

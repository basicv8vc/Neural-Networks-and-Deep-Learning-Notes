...
SGD中使用部分训练样本的偏导数来模拟损失函数的梯度，结果必然有偏差，但是注意，只要计算出的结果能保证损失函数值在减小就能保证最终找到最小值，只不过按照梯度下降方向找到最小值最快而已！所以，不需要质疑使用SGD算法得到结果的准确性。

#代码实现

Okay，终于到代码部分了。作者的代码和数据集在[Here](https://github.com/mnielsen/neural-networks-and-deep-learning)。



对于数据集这里要做下处理，从MINST训练集中选择10000张图片作为验证集，后续章节要用到，所以本章程序的训练集是原MINST训练集中的50000张图片，测试集和MINST测试集相同。
